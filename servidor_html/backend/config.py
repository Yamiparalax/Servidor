...
class Config:
    # Informações básicas
    NOME_SERVIDOR = "Servidor_HTML"
    NOME_AUTOMACAO = "novo_servidor_web"
    NOME_SCRIPT = "servidor_web"
    TZ = ZoneInfo("America/Sao_Paulo")
    
    # Modo headless
    _env_headless = os.getenv("SERVIDOR_HEADLESS", "1").strip().lower() # Default to headless for web backend
    HEADLESS = _env_headless in {"1", "true", "yes", "sim"}
    
    # Modo offline
    _env_offline = os.getenv("SERVIDOR_OFFLINE", "True").strip().lower()
    SERVIDOR_OFFLINE = _env_offline in {"1", "true", "yes", "sim"}

    # Caminhos do sistema (Ajustados para profundidade novo_servidor/servidor_html/backend)
    BASE_SERVIDOR_DIR = Path(__file__).resolve().parent.parent.parent
    ...
    DIR_LOGS_BASE = _path_from_env(
        "SERVIDOR_LOG_DIR",
        Path(os.environ.get("TEMP", os.environ.get("TMP", "/tmp"))) / "Servidor_Logs_Web",
    )
